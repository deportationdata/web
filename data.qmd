...
This is our newest dataset, covering ICE actions from mid-November 2023 through mid-February 2025. The dataset was obtained through a Freedom of Information Act lawsuit against ICE brought by UCLA’s Center for Immigration Law and Policy, represented by Kelly Dermody and Michael Levin-Gesundheit of Lieff Cabraser Heimann & Bernstein LLP. In this dataset, ICE has provided unique identifiers that correspond to individuals, allowing them to be followed anonymously through the stages of the enforcement process. Unfortunately, these identifiers do not match across releases, meaning that individuals in this dataset cannot be matched to individuals in the dataset released earlier, which covers 2012 through November 2023. This dataset also includes different categories of data. We urge particular caution in using the detention dataset because it only includes book-outs that occurred after November 15, 2023; this means that some information concerning individuals detained at the beginnning of the data may be missing. As always, but particularly for new releases, we welcome any thoughts from users about issues in the data. 
...
df_list |> 
  # filter to just get the detainers by type
  filter(Type == "courts_0303" & !str_detect(tolower(Filename), "tbllookup")) |> 
  arrange(Start) |>
