...
data_url <- "http://data.deportationdata.org"

library(googlesheets4)
df_list <- read_sheet("https://docs.google.com/spreadsheets/d/19ExRhP2OSdLLBQiI_9-V321IA536inpCf4j5b3La6kA/edit?gid=0#gid=0") 

sources <- read_sheet("https://docs.google.com/spreadsheets/d/19ExRhP2OSdLLBQiI_9-V321IA536inpCf4j5b3La6kA/edit?gid=500897663#gid=500897663", sheet = "sources") 

df_list <-
  df_list |> 
  left_join(sources, by = "Location") |> 
  mutate(
    # url_raw = glue::glue("{data_url}/{Location}/{tools::file_path_sans_ext(Filename)}_raw.{tools::file_ext(Filename)}"),
    # url = glue::glue("{data_url}/{Location}/{tools::file_path_sans_ext(Filename)}"),
    url = glue::glue("{data_url}/{Location}/{Filename}"),
    # Download = glue::glue("<a href='{url_raw}'>Original</a> <a href='{url}.xlsx'>Excel</a> <a href='{url}.csv'>CSV</a> <a href='{url}.dta'>Stata</a> <a href='{url}.rds'>R</a> <a href='{url}.dta'>SPSS</a>"),
    Source = glue::glue("<a href='{source_URL}'>{source_label}</a>"),
    Download = glue::glue("<a href='{url}'>Raw</a> ({tools::file_ext(Filename)}, {Filesize})"),
  ) 
...
## Immigration and Customs Enforcement (with Unique Identifiers)
...
## Immigration and Customs Enforcement (without Unique Identifiers)
...
df_list |> 
  # filter to just get the detainers by type
  filter(Type == "arrests_historical") |> 
  arrange(Start) |>
  transmute(Start, End, Source, Download) |> 
  reactable::reactable(columns = col_defs, pagination = FALSE)
```
...
df_list |> 
  # filter to just get the detainers by type
  filter(Type == "detainers_historical") |> 
  arrange(Start) |>
  transmute(Description, Start, End, Source, Download) |> 
...
df_list |> 
  # filter to just get the detainers by type
  filter(Type == "facilities_historical") |> 
  arrange(Start) |>
  transmute(Start, End, Source, Download) |> 
  reactable::reactable(columns = col_defs, pagination = FALSE)
```
...
df_list |> 
  # filter to just get the detainers by type
  filter(Type == "encounters_historical") |> 
  arrange(Start) |>
  transmute(Start, End, Source, Download) |> 
  reactable::reactable(columns = col_defs, pagination = FALSE)
```
...
```{r}
df_list |> 
  filter(Type == "removals_historical") |> 
  arrange(Start) |>
  transmute(Start, End, Source, Download) |> 
  reactable::reactable(columns = col_defs, pagination = FALSE)
```
...
This dataset includes apprehensions only of family units. The same apprehensions are included in some of the tables above. We post it here because it includes some fields that those datasets lack.
...
The Executive Office for Immigration Review at the Department of Justice collects information on every immigration court case, including information on scheduling, case type, immigration charges, applications for relief, and appeals. The datasets includes records from the 1970s to the last year, though most are much more recent. The agency posts an updated dataset monthly in their [FOIA Library](https://www.justice.gov/eoir/foia-library-0). 
...
df_list |> 
  ...
  arrange(Start) |>
  transmute(Description, `Release date` = End, Source, Download) |> 
  reactable::reactable(columns = col_defs_description, pagination = FALSE)
```
